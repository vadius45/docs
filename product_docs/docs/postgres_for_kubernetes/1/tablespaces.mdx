...
persistent volume. EDB Postgres for Kubernetes takes care of generating the PVCs, mounting
...
EDB Postgres for Kubernetes will create the above persistent volume claims for each instance
...
If you define a tablespace with an owner that doesn't exist, EDB Postgres for Kubernetes will
...
EDB Postgres for Kubernetes automatically handles backup of tablespaces (and the relative
...
Currently, tablespaces cannot be removed from an existing EDB Postgres for Kubernetes
cluster.
